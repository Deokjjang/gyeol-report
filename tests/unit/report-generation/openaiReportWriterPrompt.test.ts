...
describe("OpenAI report writer prompt", () => {
  it("builds Saju-first instructions with the evidence packet JSON", () => {
    ...
    expect(combined).toContain("공통 만세력표, featureDictionary, selected evidence에 실제로 있는 feature만 사용한다");
    expect(combined).toContain("feature는 같은 층위로 섞지 말고");
    expect(combined).toContain("일주 구조, 오행 구조, 십성 구조, 신살·귀인, 합충형파해, 지장간, 구조 판단");
    expect(combined).toContain("오행 분포는 별도 생활 에너지 해석으로 풀어라");
    expect(combined).toContain("화 0, 수 0처럼 비어 있는 오행");
    ...
    expect(combined).toContain("selectedMbtiKnowledge의 label, seed label, 단서 이름은 사용자 문장에 그대로 쓰지 말고");
    expect(combined).toContain("명리 구조가 MBTI 성향에서 어떻게 드러나는지");
    expect(combined).toContain("MBTI topic traits는 섹션별로 다르게 골라라");
    expect(combined).toContain("비효율을 못 넘김");
    expect(combined).toContain("논쟁형 친밀감");
    expect(combined).toContain("위임보다 직접 통제");
    expect(combined).toContain("work는 career/workplace/money/study");
    expect(combined).toContain("검증된 interaction이 있을 때만 연결");
    expect(combined).not.toContain("이런 연결 문장을 최소 4개 이상");
    ...
    expect(combined).toContain("조립문 금지");
    expect(combined).toContain("조사 오류 금지");
    ...
    expect(combined).toContain("홍염살, 육해살 같은 항목은 실제 evidence에 있을 때만 쓴다");
    expect(combined).toContain("DB item label을 붙여 '단서'처럼 노출하지 않는다");
    ...
    expect(combined).toContain("일·돈·공부는 프로젝트, 포트폴리오, 외부 제안, 수익화, 정산일");
    expect(combined).toContain("love_relationships에는 상대가 서운함을 말하는 장면");
    expect(combined).toContain("연애·관계는 말의 온도, 애정 확인 방식");
    expect(combined).toContain("people_family_environment에는 가족 부탁, 팀 역할 분담");
    expect(combined).toContain("사람·가족·환경은 가족 부탁, 팀 역할, 친구 고민");
    expect(combined).toContain("리스크·성장은 밤 산책, 기록, 수면, 물 마시기");
