  ...
  return {
    ...
    body:
      chapterId === "final_message"
        ...
        : `${titleKo}에서는 갑목과 갑신일주를 1차 근거로 삼고 ENTJ는 보조 근거로 연결합니다. 덕민님, ${titleKo}에서 상대가 아직 고민 중인데 이미 답이 보이는 상황 많지 않나요? ${titleKo}의 갑목은 방향을 세우고 앞으로 밀고 가려는 힘이라서 결론을 빠르게 잡는 모습으로 나타납니다. ${titleKo}에서 갑신일주는 압박 속에서도 기준을 지키려는 구조라서, 같은 구조라도 회의와 메시지, 일과 관계와 돈에서는 전혀 다른 장면으로 드러납니다. ${titleKo}을 MBTI 언어로 번역하면 ENTJ식 효율, 목표, 역할 정리, 빠른 결론과 해결 중심이 이 흐름을 보조합니다. 그래서 ${titleKo}의 조언은 막연한 위로보다 무엇을 기준으로 삼고 어디서 힘을 뺄지 정하는 쪽이어야 합니다. ${titleKo}에서는 결론을 바로 말하기 전에 질문을 하나 넣는 루틴을 두어야 합니다. ${titleKo}은 용어를 나열하는 칸이 아니라 실제 선택과 말투와 돈 쓰는 방식으로 사주 구조를 읽는 챕터입니다. ${titleKo}의 본문은 사용자가 하루 중 어떤 순간에 강해지고 어떤 순간에 과열되는지 떠올릴 수 있게 충분히 길고 구체적으로 이어져야 합니다. ${topicExtra}`,
    solutionLines,
