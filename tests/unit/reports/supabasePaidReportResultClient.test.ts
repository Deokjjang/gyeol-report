  ...
  const extra =
    chapterId === "work_money_study"
      ...
      : chapterId === "love_relationships"
        ? "관계에서 써먹을 것은 보완하는 사람 기준입니다. 정서적 완충이 되고 감정 표현을 부드럽게 풀어주는 사람이 맞는 사람일 수 있습니다. 피해야 할 패턴은 감정 기복이 크고 책임이 흐릿한 사람입니다. MBTI 관계 기준은 감정을 천천히 풀어주고 생활 리듬과 책임감이 안정적인 성향을 보되 MBTI만으로 단정하지 않는 태도입니다."
  ...
  return {
    ...
    body:
      isFinal
        ...
        : `${titleKo}에서는 갑목과 갑신일주를 먼저 놓고 ENTJ는 보조로만 연결합니다. 덕민님, ${titleKo}에서 상대가 아직 설명 중인데 이미 답이 보이는 상황 자주 나오지 않나요? ${titleKo}의 갑목은 방향을 세우는 힘이고, ${titleKo}의 갑신일주는 압박 속에서 기준을 잡는 구조입니다. ${titleKo} 본문은 저장된 V2 snapshot을 조회한 뒤 화면에서 narrative chapter로 렌더링할 수 있게 충분한 길이를 갖습니다. ${titleKo}에서는 회의와 메시지, 돈과 공부, 관계와 환경이 같은 사주 구조에서 출발하더라도 서로 다른 장면으로 드러난다고 설명합니다. ${titleKo}을 MBTI 언어로 번역하면 ENTJ식 효율, 목표, 역할 정리, 빠른 결론과 해결 중심이 이 흐름을 보조합니다. 그래서 ${titleKo}은 짧은 분석표가 아니라 사용자가 자기 행동을 떠올릴 수 있는 긴 호흡의 해석으로 유지되어야 합니다. ${titleKo}에서는 결론을 바로 던지기보다 질문을 먼저 넣는 루틴을 두어야 합니다. ${extra}`,
    solutionLines:
      chapterId === "opening"
        ? []
