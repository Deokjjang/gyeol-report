...
describe("LoveMarriageChildReportView source", () => {
  ...
  it("renders selected relationship content without a second hardcoded recommendation path", () => {
    expect(source).toContain("section: draft.attractionPattern");
    expect(source).toContain("section: draft.loveFriction");
    expect(source).not.toContain("INTJ·INTP·ENFP·ISFP");
    expect(source).not.toContain("buildRelationshipFitGroups");
    expect(source).not.toContain("buildRelationshipFatigueGroups");
  });
