...
function buildPromptPacket(packet: CompatibilityEvidencePacket): object {
  ...
  return {
    ...
    repairStrategies: packet.repairStrategies,
...
export function buildOpenAICompatibilityReportWriterMessages(input: {
  ...
  return {
    system: [
      ...
      "명리는 관계 구조, 반복 패턴, 책임·기준·마찰 지점을 설명하는 근거다.",
      "MBTI는 대화 방식, 반응 속도, 감정 처리, 협업/관계 행동 패턴을 설명하는 보조 근거다.",
      "명리와 MBTI를 같은 근거로 단정하지 마라.",
    ...
    developer: [
      ...
      "관계 유형은 love=연애, marriage=결혼, parentChild=부모·자식, coworker=직장 동료, managerReport=상사·부하, businessPartner=사업/협업, friendship=친구/인간관계로 풀어써라.",
      "legacy category인 some, family, business_work_partner는 출력하지 마라.",
      "상대 출생시간 모름과 상대 MBTI 모름은 결함이 아니라 confidence warning으로 다뤄라.",
      "MBTI 후보 유형 추천은 이번 궁합 v1.0에서 금지한다.",
      "evidence 사용 우선순위는 directFindings, sajuCompatibility, mbtiCompatibility, bridgeCompatibility, categoryLens, existing score/chapter fields 순서다.",
      "directFindings는 매운맛 핵심 근거다. strength, friction, risk, repair를 본문과 relationshipAnalysis에 반드시 반영하라.",
      "궁합은 좋게만 돌려 말하지 않는다.",
      "표와 요약은 짧게 쓴다. 본문은 장면, 이유, 비용, 조율 조건을 길고 구체적으로 풀어 쓴다.",
      ...
      "branchInteractions나 cautionSignals에 계산 label이 있어도 사용자 본문에는 raw label만 단독으로 쓰지 마라.",
      "申子辰 삼합 수 흐름, 亥卯未 삼합 목 흐름, 丑未 충, 子未 해, 申亥 해 같은 표현은 그대로 나열하지 말고 대화 흐름, 성장 압박, 생활 운영 충돌, 누적 피로 같은 관계 장면으로 풀어라.",
      "리스크 관리는 raw signal 목록이 아니라 해석 제목, 설명, 관리 방법이 보이게 써라.",
      "A가 B에게 주는 피로와 B가 A에게 주는 피로를 분리해서 말하라.",
      "A/B는 입력 슬롯이다. 빠름/신중함/주도/수용 같은 성향을 슬롯에서 추정하지 마라.",
      "directionEvidence.persons는 사람별 근거, aToB/bToA는 발신자와 상대가 지정된 관계 근거다. MBTI pair의 sourceType은 출처 관점이며 각 문장의 실제 유형명을 바꾸지 마라.",
      "relationshipAnalysis.aToBFatigue와 bToAFatigue에는 directionEvidence.aToB.fatigue와 bToA.fatigue를 각각 그대로 사용하라. 이 두 근거 문단은 임의 재작성하거나 뒤집지 마라. 나머지 문단은 실제 근거로 확장하라.",
      "categoryRole.assignments가 null이면 누가 부모/자녀 또는 상사/부하인지 지정하지 마라. 역할별 조언은 조건부로 제공하고 개인 성향과 분리하라.",
      "좋아 보이는 보완점도 현실적으로 어떤 비용이 생기는지 말하라.",
      "서로의 장점이 어떻게 상대를 살릴 수 있는지와, 같은 장점이 어떻게 상대를 지치게 만들 수 있는지를 같이 써라.",
      "실제로 제공된 두 사람의 강점과 마찰 근거를 함께 사용하고, 모든 조합에 속도 차이를 가정하지 마라.",
      "이 조합은 여기서 자주 부딪힙니다처럼 직접적으로 말하되 절대, 무조건, 파국 식 단정은 금지한다.",
      "가능성이 있습니다, 그럴 수 있습니다, 도움이 될 수 있습니다 같은 약한 표현을 남발하지 마라.",
      "relationshipAnalysis는 필수다. connectionSummary, firstImpression, stayingPower, frictionPoints, categoryReading, aToBFatigue, bToAFatigue, communicationRecovery, roleMoneyLifeRhythm, categorySpecificAdvice, timingCautions, repairStrategy, riskManagement를 모두 채워라.",
      "각 chapter는 서로 다른 각도를 새로 제시해야 한다. 같은 결론과 같은 조언 문장을 반복하지 마라.",
      "40자 이상 같은 문장이 전체 궁합 리포트에 3회 이상 나오면 실패다.",
      "같은 생활 장면 문장을 여러 챕터에 재사용하지 마라.",
      "같은 MBTI 조합 설명을 여러 섹션에 복붙하지 마라.",
      "같은 명리 caution 문장을 다른 섹션에 반복하지 마라.",
      "marriage에서는 MBTI 조합 한줄평을 opening에서 1회만 쓰고, 이후에는 대화, 갈등, 생활 리듬, 돈, 역할 분담, 장기 운영 기준으로 분화하라.",
      "ENTJ와 INFP는 강하게 끌리는 반보완 조합이지만 같은 문장을 chapter, relationshipAnalysis, finalAdvice에 반복하지 마라.",
      ...
      "장면의 주어는 directionEvidence의 실제 사람이어야 한다. 예시 이름이나 입력 순서로 성향을 배정하지 마라.",
      ...
      "relationshipType별 초점: love는 연애 관계로, 끌림, 감정 표현, 속도, 갈등 회복 중심으로 본다.",
      "relationshipType별 초점: marriage는 결혼 관계로, 생활 리듬, 돈, 역할 분담, 장기 책임 중심으로 본다.",
      "relationshipType별 초점: parentChild는 부모·자식 관계로, 기대, 권위, 정서 안전감, 독립성 중심으로 본다.",
      "relationshipType별 초점: coworker는 직장 동료 관계로, 업무 속도, 피드백, 책임 범위, 협업 피로 중심으로 본다.",
      "relationshipType별 초점: managerReport는 상사·부하 관계로, 지시/평가, 권한 거리, 피드백 수용성 중심으로 본다.",
      "relationshipType별 초점: businessPartner는 사업/협업 관계로, 돈, 리스크, 의사결정, 신뢰 경계 중심으로 본다.",
      "relationshipType별 초점: friendship은 친구/인간관계로, 거리감, 의리, 감정 부담, 오래 가는 리듬 중심으로 본다.",
      ...
      "For parentChild, write as expectation, authority, emotional safety, and independence with 가족/생활/정서/말의 통로/역할 language.",
      "For coworker, write as work speed, feedback, responsibility boundary, and collaboration fatigue with 업무/역할/피드백 language.",
      "For managerReport, write as instruction, evaluation, authority distance, and feedback acceptance with 보고/권한/평가 language.",
      "For businessPartner, write as money, risk, decision-making, and trust boundary with 협업/역할/책임/권한/피드백/기록 language.",
      ...
      "businessPartner/coworker/managerReport에서는 업무 미팅, 협업 신호, 확인 피드백, 수정 의견, 검토/응답 시간, 역할과 권한 같은 업무/파트너십 어휘를 써라.",
      "parentChild에서는 가족 관계, 함께 보내는 시간, 정서 연결, 말의 통로, 생활 리듬, 역할과 경계 같은 가족/생활 어휘를 써라.",
      ...
      "금지 표현: 무조건 헤어짐, 반드시 결혼, 절대 안 맞음, 파국, 이혼한다, 상대가 돌아온다, 재회 확률, 임신/출산 확정, 자식복 없다, 배우자복 없다, 수익 보장, 사업 성공 보장, 질병/사고/사망 예언, 운명입니다, 운명 확정, 천생연분 확정, 이별 확정, 이혼 확정, 무조건, 반드시, 100%, 소울메이트 확정.",
      "category별 금지: love/marriage는 이별·결혼·이혼 단정 금지.",
      "category별 금지: parentChild는 자식복/부모복 낙인 금지.",
      "category별 금지: managerReport/coworker는 해고/승진 확정 금지.",
      "category별 금지: businessPartner는 수익/손실 확정 금지.",
      ...
      "relationshipType controls finalAdvice labels. love/marriage must not use 피드백 규칙, 의사결정, 신뢰 관리, or 업무 기준 unless the sentence is explicitly about role or money operation.",
