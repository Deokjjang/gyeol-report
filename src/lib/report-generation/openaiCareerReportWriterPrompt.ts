...
function buildPromptPacket(packet: CareerReportEvidencePacket): object {
  return {
    ...
    elementCounts: packet.elementCounts,
    ...
    mbtiSourceSelection: packet.mbtiSourceSelection,
    bridgeEvidence: {
      ...packet.bridgeEvidence,
      ...Object.fromEntries((["primaryEvidence", "supportingEvidence", "cautionEvidence"] as const).map((key) => [key,
        packet.bridgeEvidence[key].map((item) => ({ ...item, evidence: { ...item.evidence,
          mbtiEvidence: { ...item.evidence.mbtiEvidence, traits: item.evidence.mbtiEvidence.traits.map(({ source: _source, ...trait }) => {
            void _source; return trait;
          }) },
        } })),
      ])),
    },
...
export function buildOpenAICareerReportWriterMessages(input: {
  ...
  return {
    system: [
      ...
      "Each careerTiming row must retain its yearBasis year, ganji and tenGod. These are review perspectives, not predictions of success.",
    ...
    developer: [
      ...
      formatList([
        ...
        "careerTiming: exactly the five distinct years supplied in timingHints, with their calculated basis",
      ...
      "Use only the customer's mbtiSourceSelection and claim-specific recommendation evidence. A source's example job is not a required career choice.",
      ...
      formatList([
        ...
        "Describe decision speed, spending, loss response and risk limits. Do not turn personality into asset suitability or recommend an asset class.",
      ...
      "For each recommendation, use its own evidenceIds, role and environment. Do not reuse an element, ten-god strength or shinsal claim from a different item.",
