...
export function buildOpenAIComprehensiveReportWriterMessages(input: {
  ...
  return {
    ...
    developer: [
      ...
      "각 장은 다른 현실 장면, 다른 행동 기준, 다른 표현으로 써라.",
      "예: 회의나 카톡 설명을 듣다가 틀린 부분이 먼저 보이는 장면은 한 번만 쓰고, 다른 장에서는 돈, 가족 부탁, 공부, 밤 루틴처럼 다른 장면으로 바꿔라.",
      "일주 설명을 [일주]은 단순한 이름이 아니라 같은 문장 틀로 반복하지 마라.",
      "갑신일주은, 정축일주은처럼 조사 오류가 있는 문장을 절대 쓰지 마라.",
      "prompt의 좋은 예 문장은 방향 참고용이다. 사용자 draft에 그대로 복사하지 말고 장면과 표현을 새로 써라.",
      ...
      "너는 version, productType, openingTitle, openingSummary, coreLine, chapters, longformReadings, finalAdvice, safetyNotes만 출력한다.",
    ...
    user: [
      ...
      "longformReadings는 opening, baseSajuReading, sajuFeatureReading, mbtiReading, sajuMbtiBridgeReading, workMoneyStudyReading, loveRelationshipReading, peopleFamilyEnvironmentReading, riskGrowthReading, finalMessage 10개를 정확히 포함한다.",
      "각 longformReading에는 readingId, titleKo, body, linkedChapterIds, sajuTermsUsed, mbtiTermsUsed를 쓴다. body는 최소 600자, 전체 6,000자 이상으로 쓰되 같은 문장으로 분량을 채우지 않는다. 사주 근거와 각 주제의 실제 장면, 실행 기준을 구체적으로 연결한다.",
      ...
      "사주 원국 요약과 MBTI 입력 요약은 화면에서 따로 정리되므로 JSON에는 narrative chapters와 longformReadings를 모두 충실히 쓴다.",
...
export function buildOpenAIComprehensiveReportRepairMessages(input: {
  ...
  return {
    ...
    user: [
      ...
      "version, productType, openingTitle, openingSummary, coreLine, chapters, longformReadings, finalAdvice, safetyNotes만 출력한다.",
      "longformReadings 10개를 누락하지 않는다. 각 body는 최소 600자, 합계 6,000자 이상이어야 하며 같은 문장 반복으로 채우지 않는다.",
