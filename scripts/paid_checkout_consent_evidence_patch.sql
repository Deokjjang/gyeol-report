-- PAID-CHECKOUT-CONSENT-EVIDENCE-P1-01
-- Apply after external-call guard patch. No backfill or new retention policy.
-- The minimal contract evidence lives once in the financial order JSON; input
-- snapshots remain linked by order_id. find_order returns their joined view.
begin;
set local lock_timeout='5s';
do $prerequisite$
begin
  if position('external_calls=v_calls' in pg_get_functiondef('public.paid_report_reliability(text,jsonb)'::regprocedure))=0 then
    raise exception 'EXTERNAL_CALL_GUARD_PATCH_REQUIRED';
  end if;
end
$prerequisite$;
