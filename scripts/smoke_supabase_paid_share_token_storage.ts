...
const paymentOrderId = "smoke_order_share_token_storage";
const paymentProvider = "smoke";
const paymentProviderPaymentId = "smoke_payment_share_token_storage";
...
const smokeInput = {
  displayName: "SUPABASE_SHARE_SMOKE",
...
function createSmokeError(message: string): Error {
  return new Error(
    `Supabase paid share token storage smoke failed: ${message}`,
  );
}
...
async function run(): Promise<void> {
  ...
  writeStatus("share path format: ok");
...
run().catch((error: unknown) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : "Supabase paid share token storage smoke failed."}\n`,
  );
  process.exitCode = 1;
});
