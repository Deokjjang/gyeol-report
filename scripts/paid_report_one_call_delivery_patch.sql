...
-- Apply AFTER checkout consent evidence. Never rerun older patches afterward.
-- No business-data backfill. Old audit rows retain their historical meaning.
begin;
set local lock_timeout='5s';
do $prerequisite$
begin
  if position('consentEvidence' in pg_get_functiondef('public.paid_report_reliability(text,jsonb)'::regprocedure))=0 then
    raise exception 'CONSENT_EVIDENCE_PATCH_REQUIRED';
  end if;
end
$prerequisite$;

alter table public.report_generation_attempts add column if not exists delivery_audit jsonb not null default '{}';
...
begin
  if p_action = 'create_order' then
    ...
    values(p_data->>'paymentOrderId',p_data->>'productType',p_data->>'provider',(p_data->>'amount')::integer,'KRW','ready',jsonb_strip_nulls(jsonb_build_object('consentEvidence',p_data->'inputSnapshot'->'consentEvidence')),p_data->>'providerOrderId',now(),now(),now()) returning * into o;
    insert into public.report_input_snapshots(order_id,payload_json) values(o.payment_order_id,(p_data->'inputSnapshot')-'consentEvidence');
    ...
    return jsonb_build_object('ok',true,'order',to_jsonb(o)||jsonb_build_object('input_snapshot',coalesce(v_input,'{}')||jsonb_strip_nulls(jsonb_build_object('consentEvidence',o.input_snapshot->'consentEvidence'))));
    ...
    insert into public.report_generation_attempts(job_id,report_id,run_number,attempt,strategy,writer_model)
      values(j.job_id,j.report_id,j.run_number,j.attempt_count,
        case when j.attempt_count=1 then 'normal_writer' else 'deterministic_fallback' end,
        case when j.attempt_count=1 then p_data->>'model' else null end);
    ...
    if p_data->>'success'='true' then
      ...
      v_status:=case when j.attempt_count>=3 or p_data->>'code' in ('PREFLIGHT_CONTRACT_INVALID','FALLBACK_INVARIANT_FAILED','ANNUAL_PURCHASE_CONTEXT_INVALID') then 'FAILED_REQUIRES_ATTENTION' else 'RETRYING' end;
    ...
    update public.report_generation_attempts set finished_at=now(),duration_ms=(p_data->>'durationMs')::integer,
      external_calls=v_calls,
      delivery_audit=jsonb_strip_nulls(jsonb_build_object(
        'version','paid-one-call-v1',
        'preflight',case when p_data->'delivery'->>'preflight' in ('pass','fail') then p_data->'delivery'->>'preflight' else null end,
        'writerValidation',case when p_data->'delivery'->>'writerValidation' in ('not_run','pass','fail') then p_data->'delivery'->>'writerValidation' else null end,
        'fallbackUsed',p_data->'delivery'->>'fallbackUsed'='true',
        'publish',case when p_data->>'success'='true' then 'pass' else 'fail' end,
        'failureCode',case when p_data->'delivery'->>'failureCode' ~ '^[A-Z][A-Z0-9_]{0,79}$' then p_data->'delivery'->>'failureCode' else null end,
        'issues',coalesce((select jsonb_agg(v) from jsonb_array_elements_text(case when jsonb_typeof(p_data->'delivery'->'issues')='array' then p_data->'delivery'->'issues' else '[]'::jsonb end) as x(v) where v ~ '^[A-Z][A-Z0-9_]{0,79}$'),'[]'::jsonb),
        'rescueKinds',coalesce((select jsonb_agg(v) from jsonb_array_elements_text(case when jsonb_typeof(p_data->'delivery'->'rescueKinds')='array' then p_data->'delivery'->'rescueKinds' else '[]'::jsonb end) as x(v) where v in ('CANONICAL_PRESENTATION','RAW_SAJU_LABEL_EXPLANATION_MISSING','EVERYDAY_SCENE_MISSING')),'[]'::jsonb)
