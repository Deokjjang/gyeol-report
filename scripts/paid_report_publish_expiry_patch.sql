-- PAID-REPORT-90DAY-EXPIRY-P1-01
-- Apply AFTER quarantine recovery, then payment confirm recovery queue patches.
-- SQL Editor only; no db push, history repair, legacy conversion or automatic backfill.
begin;
...
begin
  if not exists (select 1 from pg_constraint where conrelid='public.paid_report_snapshots'::regclass
    and conname='paid_report_snapshots_publication_check' and convalidated)
    or not exists (select 1 from information_schema.columns where table_schema='public'
      and table_name='payment_orders' and column_name='recovery_attention_at')
    or position('claim_payment_recovery' in pg_get_functiondef('public.paid_report_reliability(text,jsonb)'::regprocedure))=0 then
    raise exception 'RECOVERY_PATCHES_REQUIRED';
  end if;
end
$prerequisite$;

-- Acquire the table lock before checking existing rows. Never guess an old row's
-- first valid publication or silently change its existing retention/access promise.
alter table public.paid_report_snapshots alter column expires_at drop default;
alter table public.paid_report_snapshots alter column expires_at drop not null;
do $expiry_contract$
begin
  if exists (select 1 from public.paid_report_snapshots where not (
    ...
    or (published_at is not null and expires_at is not null
      and expires_at=published_at+interval '2160 hours')
  )) or exists (select 1 from public.paid_report_snapshots s
    join public.report_input_snapshots i using(order_id)
    where s.published_at is not null and i.expires_at is distinct from s.expires_at
  ) then
    raise exception 'EXISTING_REPORT_EXPIRY_REVIEW_REQUIRED';
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.paid_report_snapshots'::regclass
    and conname='paid_report_snapshots_access_expiry_check') then
    alter table public.paid_report_snapshots add constraint paid_report_snapshots_access_expiry_check check (
      (published_at is null and expires_at is null and status<>'COMPLETED' and snapshot_json is null)
      or (published_at is not null and expires_at is not null
        and expires_at=published_at+interval '2160 hours')
    );
