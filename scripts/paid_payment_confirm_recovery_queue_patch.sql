-- PAID-PAYMENT-CONFIRM-RECOVERY-QUEUE-P1-01
-- Apply AFTER paid_report_quarantine_recovery_patch.sql, BEFORE the new worker.
-- Manual SQL Editor patch; no migration history repair/db push/backfill.
begin;
...
begin
  if not exists (select 1 from pg_constraint where conrelid='public.paid_report_snapshots'::regclass
    ...
    or exists (select 1 from pg_constraint where conrelid='public.paid_report_snapshots'::regclass
      and conname='paid_report_snapshots_check') then
    raise exception 'QUARANTINE_PATCH_REQUIRED';
  end if;
end
$prerequisite$;
...
alter table public.payment_orders
  ...
  add column if not exists recovery_next_retry_at timestamptz not null default now(),
  add column if not exists recovery_last_error_code text,
  add column if not exists recovery_attention_at timestamptz;
create index if not exists payment_orders_recovery_due_idx
  on public.payment_orders(recovery_next_retry_at,created_at,payment_order_id)
  where provider='toss' and status in ('ready','paid') and deleted_at is null and recovery_attention_at is null;
